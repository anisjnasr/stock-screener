...
import { discoverAndStoreThemeMarketEvents } from "@/lib/ai/themeEventSearch";
...
/**
 * For each `daily_themes` row, web-search scheduled events and upsert `market_events` (`theme_driven`).
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const result = await discoverAndStoreThemeMarketEvents(supabase, anthropic, { ymd });
    if (!result.ok) {
    ...
    console.error("[theme-event-search]", message);
