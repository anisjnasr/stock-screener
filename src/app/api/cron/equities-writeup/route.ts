...
import { generateAndStoreDailyEquitiesWriteup } from "@/lib/ai/equitiesWriteup";
...
/**
 * Build `daily_equities_writeup` from `newsletter_archive` (or web-search fallback).
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const result = await generateAndStoreDailyEquitiesWriteup(supabase, anthropic, { ymd });
    if (!result.ok) {
    ...
    console.error("[equities-writeup]", message);
