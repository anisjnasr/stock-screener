...
/** One row per symbol for premarket movers join (latest quote_daily date + indicators avg vol). */
export type PremarketScreenerJoinRow = {
  symbol: string;
  name: string | null;
  marketCap: number | null;
...
/**
 * Full screener universe for joining Polygon full-market snapshot in memory (no symbol cap).
 * Returns an empty map when the DB is missing or has no quote date.
 */
export function getPremarketScreenerJoinMap(): {
  byTicker: Map<string, PremarketScreenerJoinRow>;
  date: string | null;
} {
  const db = getDb();
  if (!db) return { byTicker: new Map(), date: null };
  const date = getLatestScreenerDate();
  if (!date) return { byTicker: new Map(), date: null };
  const sql = `
    ...
    LEFT JOIN indicators_daily i ON i.symbol = c.symbol AND i.date = q.date
  `;
  const rows = db.prepare(sql).all(date) as Array<{
    symbol: string;
    name: string | null;
    market_cap: number | null;
    avg_volume_1m: number | null;
  }>;
  const byTicker = new Map<string, PremarketScreenerJoinRow>();
  for (const r of rows) {
    const sym = String(r.symbol).toUpperCase();
    const marketCap =
      typeof r.market_cap === "number" && Number.isFinite(r.market_cap) && r.market_cap > 0 ? r.market_cap : null;
    const avgVolume1m =
      typeof r.avg_volume_1m === "number" && Number.isFinite(r.avg_volume_1m) && r.avg_volume_1m > 0
        ? r.avg_volume_1m
        : null;
    byTicker.set(sym, {
      symbol: sym,
