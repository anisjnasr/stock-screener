#!/usr/bin/env node
/**
 * POST all four /api/cron/market-events/* routes (Fed, Treasury, White House, USTR → Supabase).
 *
 * Loads optional `.env.local` from repo root (same pattern as trigger-economic-calendar).
 *
 * Usage:
 *   npm run market-events:trigger
 *   npm run market-events:trigger -- --url https://your-app.onrender.com
 *
 * Env (or .env.local):
